...
- **Root Directory**: /home/blandland/public_html
- **FTP**: ftp.blacksitedb.com
- **User**: blandland
- **Databases**:
  - blandland_ticket_storm_users
  - blandland_ticket_storm_tickets
- **Database User**: blandland_admin
...
2. Upload all files from the `ticketstorm` folder to `/home/blandland/public_html/`
...
3. Import the database structure:
   - Select database `blandland_ticket_storm_users`
   - Click "Import"
...
```bash
# Connect to MySQL
mysql -u blandland_admin -p

# Run the SQL file
source /home/blandland/public_html/database/setup.sql
```
...
define('DB_USER', 'blandland_admin');
define('DB_PASS', 'The  hater#');  // Update if needed
define('DB_USERS', 'blandland_ticket_storm_users');
define('DB_TICKETS', 'blandland_ticket_storm_tickets');
```
...
```sql
USE blandland_ticket_storm_users;
...
1. Database (both `blandland_ticket_storm_users` and `blandland_ticket_storm_tickets`)
...
mysqldump -u blandland_admin -p blandland_ticket_storm_users > backup_users.sql

# Backup tickets database
mysqldump -u blandland_admin -p blandland_ticket_storm_tickets > backup_tickets.sql
```
...
1. Check error logs: `/home/blandland/public_html/error_log`
